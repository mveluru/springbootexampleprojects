...
`org.bee.banking.gateway` implements a lightweight API-gateway ingress layer in front of *every* banking controller path (`/v1/api/accounts/*`, `/v1/client/*`, `/v1/payment/*`, `/notify`, `/notify-sms`, `/report` — see `BankingGatewayConfig.BANKING_URL_PATTERNS`, shared by both filters below). Neither filter is `@Component` — annotating either directly would make Spring Boot also auto-register it for `/*` — so both are registered via their own `FilterRegistrationBean` in `BankingGatewayConfig`. If you add a new banking controller or endpoint, add its path to `BANKING_URL_PATTERNS` or it won't get a `btid`/won't be rate-limited.

Two filters run in a fixed order (`BusinessTransactionIdFilter` at `Ordered.HIGHEST_PRECEDENCE`, `BankingRateLimitFilter` at `HIGHEST_PRECEDENCE + 1` — lower value runs first):
...
The banking module's `AccountRepositoryTest`, `ClientAccountServiceTest`, `AccountStatusStatementServiceTest`, `CustomerRateLimiterTest`, `BankingRateLimitFilterTest`, and `BusinessTransactionIdFilterTest` (`src/test/java/org/bee/banking/...`) are deliberately plain JUnit 5 + Mockito unit tests with **no** `@SpringBootTest` — none of `AccountRepository`, `ClientAccountService`, `AccountStatusStatementService`, `CustomerRateLimiter`, `BankingRateLimitFilter`, or `BusinessTransactionIdFilter` have Spring/MySQL dependencies of their own (in-memory maps/counters, plain-POJO collaborators, mocked `HttpServletRequest`/`Response`/`FilterChain`, direct `MDC` reads), so these run standalone even without MySQL up. When adding tests for banking business logic (validation rules, status transitions, balance math, rate limiting), prefer this pattern over booting the full context; reserve `@SpringBootTest`/`MockMvc` for controller-layer/wiring tests.
