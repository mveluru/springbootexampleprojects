...
Tests use `@SpringBootTest` + `MockMvc` (see `ProductControllerTest`, `BriteConfigValuesControllerTest`, `SpringBootProjectsApplicationTests`). Because the full context boots, every test run needs MySQL reachable as described above.
